...
```
  ...
  explan : create mpg01 and Assign 1 
          if mpg is above the median then combine to Auto the mpg01
...
explan : the of plots, `mpg01`, `Weight`, `Cylinders`, `Displacement`,`Horsepower`.
...
```
  Exaplan :the cheack for the ever years split train data-210 and test data-182 records
...
```
  Explan : The  4 predictors (`cylinders`, `weight`, `displacement` ,`horsepower`) with in a Linear Discriminant               Analysis the test error rate is 12.63736%.
...
```
  Explan: Using all 4 predictors (`cylinders`, `weight`, `displacement`, and `horsepower`) with a Quadratic                    Discriminant Analysis the test error rate is 13.18681%
...
```
  explan : The  4 predictors (`cylinders`, `weight`, `displacement`, `horsepower`) with a Logistic                      Regression Analysis the test error rate is 12.08791%.
...
Explan :KNN Analysis (K = 1)and (k = 5), the test error rate is `r mean(knn.pred.1 != mpg01.test) * 100`%